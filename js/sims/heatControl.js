function heatControl(lessonData, simArea, revealPoint, onFinish) {
    ...
    const slider = simArea.querySelector("#heat-slider");

    const loop = setInterval(() => {
        currentTemp += (targetTemp - currentTemp) * 0.05;
        simArea.querySelector("#temp-display").innerText = Math.round(currentTemp) + "°C";

        if (currentTemp >= target - tolerance && currentTemp <= target + tolerance) {
            timeInZone++;
            if (timeInZone === 20) revealPoint();
            if (timeInZone === 60 && !finished) {
                finished = true;
                clearInterval(loop);
                onFinish();
            }
